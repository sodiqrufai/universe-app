...
export default function ApprovedVerificationsPage() {
  ...
  const fetchApproved = async () => {
    ...
    setError(null);
    try {
      const data = await adminApi.get('/admin/verifications/approved');
      if (data.success) {
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold text-foreground mb-1">Approved Verifications</h1>
      <p className="text-text-secondary mb-6">Students who have been verified.</p>

      {loading && <p className="text-text-secondary">Loading...</p>}
      {error && (
        <div className="bg-error/10 text-error rounded-lg px-4 py-3 mb-4 flex items-center justify-between">
          <span>{error}</span>
          <button onClick={fetchApproved} className="font-semibold underline">
            Retry
          </button>
        </div>
      )}
      ...
      {!loading && !error && verifications.length === 0 && (
        <p className="text-text-secondary">No approved verifications yet.</p>
      )}
      ...
      <div className="space-y-3">
        {verifications.map((v) => (
          ...
          >
            ...
            <span className="text-xs bg-success/15 text-success px-3 py-1 rounded-full font-medium">
              Approved
            </span>
          </div>
        ))}
