...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _handleTap(dynamic n) async {
    ...
    final data = n['data'];
    if (n['type'] == 'chat_message' && data != null && data['conversationId'] != null) {
      if (mounted) {
        await Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) => ChatDetailScreen(conversationId: data['conversationId'], title: n['title']),
          ),
        );
      }
    } else if (n['type'] == 'new_offer' && data != null && data['listingId'] != null) {
      if (mounted) {
        await Navigator.of(context).push(
          MaterialPageRoute(builder: (_) => ListingDetailScreen(listingId: data['listingId'])),
        );
      }
    }
    _fetch();
  ...
  IconData _iconFor(String type) {
    switch (type) {
      ...
      case 'new_offer': return Icons.local_offer_outlined;
              ...
              : RefreshIndicator(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final timeLabel = DateFormat('MMM d, h:mm a').format(DateTime.parse(n['created_at']).toLocal());
                      ...
                      return ListTile(
                        ...
                        trailing: Column(
                          ...
                          children: [
                            Text(timeLabel, style: const TextStyle(fontSize: 10, color: Colors.black45)),
                            if (isUnread)
                              Container(
